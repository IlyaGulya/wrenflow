...
- **Typed UI boundary**: screens dispatch `AppAction` into `AppModel`. Runtime
  IO crosses `RuntimeCommand`, snapshots and events; screens never access the
  runtime or AppKit directly.
...
- **Icons built from SVG**: `mise run icons` generates `Resources/AppIcon.icns` from `AppIcon-Dock.svg` via resvg and iconutil.
- **Bundle metadata**: the GPUI bundle uses
  `native/wrenflow-gpui/macos/Info.plist` and `Wrenflow.entitlements`; the build
